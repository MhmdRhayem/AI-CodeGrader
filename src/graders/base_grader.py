...
from prompts import ResponseParser
...
class BaseGrader(ABC):
    ...
    def _parse_llm_response(self, response_text: str) -> dict:
        try:
            ...
            if not self.response_parser.validate_grading_response(parsed):
                ...
                raise ValueError("LLM response validation failed: Missing required fields or empty breakdown.")
    ...
    def _build_result(
        ...
        # Extract scores
        total_score = parsed_response.get("total_score", 0)
        total_possible = parsed_response.get("total_possible", request.rubric.total_points)
        percentage = parsed_response.get("percentage", 0)
        ...
        # Build breakdown
        breakdown = []
        for item in parsed_response.get("breakdown", []):
            breakdown.append(
                CriterionScore(
                    criterion_name=item.get("criterion_name", ""),
                    points_awarded=float(item.get("points_awarded", 0)),
